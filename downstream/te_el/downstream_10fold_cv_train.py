...
def main():
    parser = argparse.ArgumentParser(description="Downstream 10-fold CV training for TE/EL tasks")
    parser.add_argument('--data_path', type=str, required=True, help='CSV data path (with sequence and label columns)')
    ...
    parser.add_argument('--max_length', type=int, default=256, help='Max sequence length (pad/truncate)')
    ...
    parser.add_argument('--freeze_encoder', action='store_true', help='Freeze encoder, only train regression head')

    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--epochs', type=int, default=200, help='Number of training epochs')
    parser.add_argument('--n_splits', type=int, default=10, help='KFold splits')
    parser.add_argument('--lr', type=float, default=2e-4, help='Learning rate')
    parser.add_argument('--regressor', type=int, default=1, help='Regressor head type (1=CNN, 2=ResNet, 3=MLP)')

    parser.add_argument('--seed', type=int, default=42, help='Random seed')
    parser.add_argument('--wandb_project', type=str, default='EL_10fold_CV', help='W&B project name')
    ...
    sequences, labels = read_5putr_from_csv_regression_data(
    ...
    tokenized_data = tokenize_sequences(sequences, tokenizer, args.max_length)
    dataset = mRNA_RegressionDataset(tokenized_data, labels)

    file_base = os.path.splitext(os.path.basename(args.data_path))[0]
    file_name = file_base.split('_')[0]
    ...
    output_name = (
        f"{file_name}_{args.label_column}_{args.data_column}_"
        f"{pretrain_tag}_{freeze_tag}_epoch={args.epochs}_bs={args.batch_size}_lr={args.lr}_"
        f"regressor={args.regressor}_{args.device_tag}"
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        model, config = build_gpt2(tokenizer, args.n_embd, args.n_layer, args.n_head, args.max_length)
        ...
        lit_model = regressor_class(
            freeze_encoder=args.freeze_encoder,
            pretrained_model=model,
            lr=args.lr,
            log_name=f"{output_name}_fold{fold+1}",
            input_len=args.max_length
        )
        ...
        fold_dir = os.path.join(
            args.output_dir,
            f"{file_name}_{args.label_column}_{args.data_column}__epoch={args.epochs}"
            f"_bs={args.batch_size}_lr={args.lr}_regressor={args.regressor}_{freeze_tag}_{args.device_tag}",
            f"fold_{fold+1}"
        )
        os.makedirs(fold_dir, exist_ok=True)
        checkpoint_callback = ModelCheckpoint(
            dirpath=fold_dir,
            filename=f"gpt2-reg-{file_name}-{args.label_column}-{args.data_column}-{pretrain_tag}-"
